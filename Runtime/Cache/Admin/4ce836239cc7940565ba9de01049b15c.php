...
<div id="main-div">
    <div id="menu-list">
        <ul id="menu-ul">
            <?php if(is_array($btns)): foreach($btns as $key=>$val): ?><li class="explode" key="02_cat_and_goods" name="menu">
            <?php echo ($val["auth_name"]); ?>
                <ul>
                    <?php foreach($val['children'] as $k=>$v):?>
                    <li class="menu-item">
                      <a href="<?php echo U($v['module_name'] . '/' . $v['controller_name']. '/' .$v['action_name']);?>" target="main-frame">
